...
export default function BusinessPlan() {
  const [stats, setStats] = useState<{ self?: number; team?: number; claimed?: number[] } | null>(null);
  ...
  return (
    <div className="space-y-3">
      ...
      {[["Daily ROI (auto)", "Auto-credited on active investment"],["Daily Bet Limit (visual)", `${BUSINESS_RULES.dailyRunLimit}`],["Withdrawal Charge", `${BUSINESS_RULES.withdrawalChargePct}%`],["Income Capping", `${BUSINESS_RULES.incomeCapX}x`],["Bot Validity", "365 days"],["Crypto Gateway", BUSINESS_RULES.cryptoGateway],["AI Help Desk", BUSINESS_RULES.aiHelpDesk],["Bot Simulation Rounds", BUSINESS_RULES.botRoundsPerDay],["Reward Milestones", "20"]].map(([a, b]) => (<div key={a} className="av-card p-4"><p className="text-sm text-slate-400">{a}</p><p className="text-lg font-black">{b}</p></div>))}
      ...
      <div className="av-card p-4"><h3 className="font-bold">My progress: Self ${Number(stats?.self || 0).toFixed(2)} · Team ${Number(stats?.team || 0).toFixed(2)}</h3>
        {msg && <p className="mt-2 text-sm text-yellow-200">{msg}</p>}
        <div className="mt-2 space-y-1">{MILESTONES.map((m) => {
          const done = (stats?.self || 0) >= m.self && (stats?.team || 0) >= m.team;
          const claimed = stats?.claimed?.includes(m.tier);
          return (<div key={m.tier} className="flex items-center justify-between rounded-lg bg-black/30 px-3 py-2 text-xs"><span>{m.tier}. {m.name} · Self ${m.self} · Team ${m.team} · ${m.wallet}</span>{claimed ? <span className="font-bold text-slate-500">CLAIMED</span> : done ? <button onClick={() => claim(m.tier)} className="av-btn-yellow px-3 py-1 text-xs">Claim</button> : <span className="text-slate-500">LOCKED</span>}</div>);
        })}</div>
      </div>
    </div>
  );
