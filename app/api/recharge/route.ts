...
export async function POST(req: NextRequest) {
  ...
  const { amount, tx_hash } = await req.json();
  const settings = await getSettings();
  ...
  await db.execute({
    sql: "INSERT INTO deposits (id,user_id,request_id,requested,actual,tx_hash,status) VALUES (?,?,?,?,?,?,?)",
    args: [uid("D"), u.id as string, request_id, requested, amt, tx_hash, "pending"],
  });
